...
def plot_vswr(freq_mhz=FREQ_MHZ, wire_radius_m=DEFAULT_WIRE_RADIUS_M,
              output_path="vswr_sweep.png"):
    """Plot the 2D VSWR vs dipole length (fraction of wavelength) for a range of lengths."""
    wavelength = 300.0 / freq_mhz
    fracs = np.linspace(0.44, 0.52, 25)
    vswrs = []
    for f in fracs:
        vswrs.append(min(simulate_dipole(f * wavelength, freq_mhz, wire_radius_m, ground=False)["vswr"], 20))
...
def plot_2d_polar_comparison(r_free, r_ground, output_path="radiation_pattern_2d.png"):
    ...
    ax.plot(np.radians(90 - th_free), gain_free,
            label=f"Free Space ({r_free['max_gain_dbi']:.1f} dBi)")
    ax.plot(np.radians(90 - th_gnd), gain_gnd,
...
def plot_3d_balloon(r_data, ax, title):
    ...
    ax.plot_surface(X, Y, Z, facecolors=norm_colors, rstride=1, cstride=1, antialiased=True)
    ax.set_title(title)
    ax.set_box_aspect([1, 1, 1])

def plot_3d_comparison(r_free, r_ground, output_path="radiation_pattern_3d.png"):
